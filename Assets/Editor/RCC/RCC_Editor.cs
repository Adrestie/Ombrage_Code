	...
	public class RCC_Editor : Editor
	{
		...
		static void CreateBehavior()
		{
			...
			if (!Selection.activeGameObject.GetComponent<RCC_CarControllerV3>())
			{
				...
				Selection.activeGameObject.GetComponent<Rigidbody>().linearDamping = .05f;
				Selection.activeGameObject.GetComponent<Rigidbody>().angularDamping = .5f;
		...
		public override void OnInspectorGUI()
		{
			...
			if (DamageSettings)
			{
				...
				EditorGUILayout.Space();
				GUI.color = Color.cyan;
				EditorGUILayout.HelpBox("Energy Settings", MessageType.None);
				GUI.color = defBackgroundColor;
				EditorGUILayout.Space();

				EditorGUILayout.PropertyField(serializedObject.FindProperty("useEnergy"), new GUIContent("Use energy ?", "does the wheel use energy ?"), true);
			}
