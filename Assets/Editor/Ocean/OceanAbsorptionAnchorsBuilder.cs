// OceanAbsorptionAnchorsBuilder.cs  (outillage éditeur)
// Crée les 3 assets-ancres Jerlov du module absorption (EXACTEMENT 3 profils préchargés,
...
// le triplet σ. Passe 2026-07-06 : renforcement de l'inversion σ_B > σ_G sur
...
// contrainte à préserver si l'on baisse un σ. Révision fine toujours possible (a(λ)
...
// create-if-missing (contrairement au profil de test, volontairement recréé déterministe).
using UnityEditor;
using UnityEngine;
...
namespace Ombrage.OceanFeatures.GateTools
{
    public static class OceanAbsorptionAnchorsBuilder
    {
        ...
        public static void CreateAnchors()
        {
            ...
            if (ok)
                Debug.Log("[Ocean] Ancres d'absorption prêtes : Ia / II / III sous " + kFolder +
                          " (create-if-missing, jamais écrasées). Valeurs littérature de départ — calibrage " +
                          "colorimétrique dû. Le module Absorption les auto-résout en éditeur ; " +
                          "SAUVER le profil pour sérialiser les références (exigence build).");
            else
                Debug.LogError("[Ocean] Échec de création d'au moins une ancre d'absorption (voir logs).");
        ...
        static WaterAbsorptionProfile EnsureAnchor(string path, string label, float r, float g, float b)
        {
            ...
            if (existing != null)
            {
                Debug.Log($"[Ocean] Ancre déjà présente (inchangée) : {path}");
                return existing;
            }
            ...
            Debug.Log($"[Ocean] Ancre créée : {path}  σ=({r}, {g}, {b}) m⁻¹ — « {label} »");
            return p;
        }
    }
}
